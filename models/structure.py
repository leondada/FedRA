...
class VPT_ViT(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 VPT_type="Shallow", basic_state_dict=None, depth_cls = 0):
    ...
    def New_CLS_head(self, new_classes=15):
        if self.depth_cls>0:
            self.mid_head = nn.ModuleList([nn.Linear(self.embed_dim, new_classes) for i in range(int((self.depth-1)//self.depth_cls))])
